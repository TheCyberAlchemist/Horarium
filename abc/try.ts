...
var slots = [];
class slot{
	id;day;timing;resources_filled;
	constructor(id:number = 0,day = 0,timing = 0,resources_filled=0){
		this.id = id;
		this.day = day;
		this.timing = timing;
...
class subject_event {
	...
	color:string
	faculty_name:string
	constructor(id:number = 0,subject_name,subject_id,prac_carried,lect_carried,faculty_id,not_available
		,other_events,color,faculty_name:string){
		this.id = id;
...
class event_class {
	Slot_id:number;
	Subject_event_id:number;
	Batch_id:number;
	Resource_id:number;
	Slot_id_2:number;
	locked:boolean;
	link:string;
	put_slots(Slot_id:number,Slot_id_2:number=0){
		this.Slot_id = Slot_id;
		this.Slot_id_2 = Slot_id_2;
	}
	put_batch_resource(Batch_id:number,Resource_id:number){
		this.Batch_id = Batch_id?Batch_id:null;
		this.Resource_id = Resource_id?Resource_id:null;
		
	}
	put_link_locked(link:string,locked:boolean=false){
		this.link = link?link:null;
		this.locked = locked?locked:false;
	}
	put_subject_event(Subject_event_id:number){
...
function put_data(slots_json,sub_events_json,batches,old_events_json,subjects_json){
	for (let i in slots_json){
		let temp_slot = new slot(slots_json[i].pk,slots_json[i].fields.day,slots_json[i].fields.Timing_id,slots_json[i].fields.resources_filled)
		slots.push(temp_slot);
	}
	// console.table(sub_events_json);
	// events_json = JSON.parse(events_json.replace(/&#34;/ig,'"',));
	for (let i in sub_events_json){
		let obj = sub_events_json[i].fields;
		let temp_sub_event = new subject_event(sub_events_json[i].pk,obj.Subject_name,obj.Subject_id,obj.prac_carried,obj.lect_carried,obj.Faculty_id,obj.not_available
...
function subject_event_has_load_remaining(subject_event_id,is_prac=false){
	...
	let total_similar_events = event_arr.length;
		...
		case "removed":
			if (push_event(my_event)){
				...
				if (my_event.Slot_id_2){
					let subject_batch = get_batches(my_event.Subject_event_id,true);
					if (!td.html()){
						change_to_prac_td(td,subject_batch);
					}
					put_prac(td,my_event.Subject_event_id,my_event.Batch_id,my_event.Resource_id);
				}else{	// if lect 
					let subject_batch = get_batches(my_event.Subject_event_id,false);
					if (!td.html()){
...
function redo(){
	...
	switch(last_undo.type){
		case "added":
			temp_events = events.filter(e=> e.Slot_id != my_event.Slot_id || e.Subject_event_id != my_event.Subject_event_id);
			cleared = clear_all_unlocked_td(my_event,false,true);
			console.log(cleared);
			// events = temp_events;
			if (cleared){
				put_json_in_table(temp_events);
...
function clear_batch_div(td,batch){
	// console.log(td.find(""))
	let batch_div = td.find(`[batch_for=${batch}]`);
    let button = batch_div.find(".event_name");
    let faculty_div = batch_div.find(".faculty_name");
    let resource_div = batch_div.find(".resource_name");
    faculty_div.html("");
    resource_div.html("");
    button.html("");
...
function hotkeys(arg0: string, arg1: (event: any, handler: any) => void) {
	throw new Error("Function not implemented.");
}

function temp_event(temp_event: any) {
	throw new Error("Function not implemented.");
}

function td(td: any, arg1: boolean | any[]) {
	throw new Error("Function not implemented.");
}

function temp_slot(temp_slot: any) {
	throw new Error("Function not implemented.");
}

function temp_sub_event(temp_sub_event: any) {
	throw new Error("Function not implemented.");
}

function total_load(total_load: any) {
	throw new Error("Function not implemented.");
}

function faculty_div(faculty_div: any, resource_div: any, button: any) {
	throw new Error("Function not implemented.");
}

function resource_div(faculty_div: any, resource_div: any, button: any) {
	throw new Error("Function not implemented.");
}

function button(faculty_div: any, resource_div: any, button: any) {
	throw new Error("Function not implemented.");
}

function asd(asd: any) {
	throw new Error("Function not implemented.");
}
//#endregion
